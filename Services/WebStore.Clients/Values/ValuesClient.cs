...
namespace WebStore.Clients.Values
{
    public class ValuesClient : BaseClient, IValuesService
    {
        ...
        {
            var response = await _Client.PostAsJsonAsync($"{_ServiceAddress}/post", value);  //запрос  пост предназначен для создания объекта на стороне сервиса, и мы должны передать то что хотим создать
