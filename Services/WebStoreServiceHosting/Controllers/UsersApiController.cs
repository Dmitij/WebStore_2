...
namespace WebStore.Controllers
{
    ...
    public class UsersApiController : ControllerBase
    {
        ...
        [HttpGet("AllUsers")]  //просмотр всех пользователей
        public async Task<IEnumerable<User>> GetAllUsers() => await _UserStore.Users.ToArrayAsync();

        #region Users

        [HttpPost("UserId")]  //так как сложный объект используем post
        ...
        [HttpPost("UserName/{name}")]  //изменениние имени пользователя
        ...
        [HttpPost("NormalUserName")]  //нормализация имени пользователя. работа с такими именами выполняется быстрее
        public async Task<string> GetNormalizedUserNameAsync([FromBody] User user) => await _UserStore.GetNormalizedUserNameAsync(user);

        [HttpPost("NormalUserName/{name}")] //нормализация имени пользователя. работа с такими именами выполняется быстрее
        ...
        [HttpPost("SetPasswordHash")]   //возвращает установленный хеш. PasswordHashDTO шифрует пароль внутри модели 
